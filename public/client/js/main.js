...
(function ($) {
  ...
  function addToFavorite() {
    $(this).on("click", function () {
      if ($(this).hasClass("active")) {
        ...
        const id = parseInt($product.data("id"));
        if (addToFavoriteDatabase($product[0], id)) {
          addToFavoritePreview($product[0], id);
          $(this).addClass("active");
          showToastMessage(
            "Added to favorite products: " + id,
            toastData["success"]
          );
        }
      }
  ...
  function addToFavoritePreview(product, id) {
    const maxSize = 4;
    const $dropdown = $(".header__cart__favorite .header__cart__dropdown");
    if ($dropdown.children("li").length == maxSize) {
      $(
  ...
  function addToFavoriteDatabase(product, id) {
    return true;
  ...
  $(".delete__product").on("click", async function () {
    ...
    const response = await fetch(`/carts/delete`, {
      method: "DELETE",
      headers: {
        'Content-Type': 'application/json',
      },
